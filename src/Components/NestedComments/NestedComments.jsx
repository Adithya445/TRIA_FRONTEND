...
import { ChevronDown, ChevronUp, MessageSquare, ThumbsUp, ThumbsDown, Trash2, Send, User, LogOut } from 'lucide-react';
  ...
  return (
    <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-slate-700/50">
      ...
      <div className="p-6 md:p-8">
        <h1 className="text-3xl md:text-4xl font-bold text-white mb-3 bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent">{post.title}</h1>
        <div className="flex items-center text-sm text-gray-400 mb-4">
          <User className="w-4 h-4 mr-1" />
          <span className="mr-4">{post.author}</span>
          <span>{new Date(post.createdAt).toLocaleDateString()}</span>
        </div>
        <p className="text-gray-300 leading-relaxed text-base md:text-lg">{post.content}</p>
      </div>
    </div>
  );
};
...
const CommentInput = ({ onSubmit, placeholder = "Add a comment...", autoFocus = false }) => {
  ...
  return (
    <div className="flex gap-2">
      ...
      <button onClick={handleSubmit} disabled={!text.trim()} className="self-end px-6 py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-xl hover:shadow-lg disabled:opacity-50 transition-all flex items-center gap-2 font-medium">
...
const Comment = ({ comment, onReply, onVote, onDelete, currentUser, depth = 0 }) => {
    ...
    <div className={`mt-4 ${depth > 0 ? 'ml-4 md:ml-8' : ''}`}>
      <div className="bg-slate-800/30 backdrop-blur-sm rounded-xl p-3 md:p-5 shadow-lg border border-slate-700/50">
        <div className="flex items-start justify-between mb-3">
          ...
          <button onClick={() => setIsCollapsed(!isCollapsed)} className="text-gray-500 hover:text-white p-2 rounded-full hover:bg-slate-700/50 transition-colors">
        ...
        {!isCollapsed && (
          <>
            ...
            <div className="flex items-center gap-2 md:gap-4 ml-13 flex-wrap">
              ...
              {canDelete && <button onClick={() => onDelete(comment._id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-700/30 text-gray-400 hover:text-red-400 transition-colors md:ml-auto"><Trash2 className="w-4 h-4" /> Delete</button>}
            </div>
...
const CommentThread = ({ comments, ...handlers }) => {
  const buildTree = (list) => {
    ...
    const roots = [];
    list.forEach(c => {
      if (c.parent_id && map[c.parent_id]) { map[c.parent_id].replies.push(map[c._id]); } 
...
export default function NestedComments() {
  ...
  useEffect(() => { if (user) fetchData(sortBy); }, [user, sortBy]);
  ...
  const handleDeleteComment = (commentId) => { if (window.confirm("Delete this comment and all its replies?")) { handleCommentAction(() => api.delete(`/comments/${commentId}`), "Comment deleted!"); } };
  const handleVote = (commentId, voteType) => handleCommentAction(() => api.post(`/comments/${commentId}/vote`, { vote_type: voteType }), "Vote registered!");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <Header user={user} />
      <div className="relative z-10 max-w-5xl mx-auto p-4 md:p-6 py-8">
        <Post post={post} />
        <div className="bg-slate-800/30 backdrop-blur-sm rounded-2xl shadow-2xl p-4 md:p-8 mt-6 border border-slate-700/50">
          ...
          <CommentThread comments={comments} onReply={handleAddComment} onVote={handleVote} onDelete={handleDeleteComment} currentUser={user} />
        </div>
      </div>
    </div>
  );
